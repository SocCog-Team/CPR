...
[trg_acc,trg_conf,trg_rew,trg_hit,trg_coh] = getTargetValue(t);
...
for i = 1:length(trg_ts)
    for j = 1:length(trg_ts{i})
        c           = c+1;
        trg_pos     = find(frme_ts{i} >= trg_ts{i}(j), 1, 'first');
        js_dev      = rad2deg(circ_dist(deg2rad(js_dir{i}(trg_pos)),deg2rad(rdp_dir(i))));
        acc(c)      = abs(1 - abs(js_dev / 180));
        conf(c)     = js_str{i}(trg_pos);
        coh(c)      = trg_coh(i);
        hi(c)       = trg_hit{i}(j);
    end
end
