function [d,t] = PHY_daily_summary_test2(data_pth, fname, cfg_pth, writeH5_flag, dest_dir_tbl, dest_dir_plot)
...
% Check input
if nargin < 6
    dest_dir_plot 	= '/Users/anazari/ownCloud/CPR_files/NHP_training/png/';
end

if nargin < 5
    dest_dir_tbl    = '/Users/anazari/ownCloud/CPR_files/NHP_training/mat/';
end
...
if nargin < 3
    cfg_pth         = '/Users/anazari/ownCloud/CPR_files/NHP_training/felix_nhp_solo.cfg';
end
...
addpath /Users/anazari/ownCloud/CPR_files/NHP_training
addpath /Users/anazari/ownCloud/Shared/MWorks_MatLab/
addpath /Users/anazari/Documents/MATLAB/CircStat2012a/
addpath /Users/anazari/Documents/GitHub/Violinplot-Matlab
...
ax0.XTickLabel       	= 1:length(solo_perf.carr);
ax0.XTickLabel       	= {solo_perf.carr};
ax0.XLabel.String    	= 'Coherence';
ax0.YLabel.String    	= 'Performance';
ax0.FontSize         	= fsz;
...
snr = unique(solo_perf.trg_all_coh);
for iCoh = 1:length(snr)
    clear xdat ydat
    ydat = solo_perf.trg_all_ecc(solo_perf.trg_all_coh == snr(iCoh));
    xdat = solo_perf.trg_all_acc(solo_perf.trg_all_coh == snr(iCoh));
...
ax7.XTick             	= [0:.2:1];
ax7.YTick             	= [0:.2:1];
ax7.XLabel.String     	= 'Acccuracy';
ax7.YLabel.String    	= 'Eccentricity';
ax7.FontSize          	= fsz;

% Total juice: Target vs Cycle reward
reward_time             = d.time(idx.reward);
tmp_rew                 = d.value(idx.reward);
individual_reward       = diff([0 cellfun(@double,tmp_rew)]);
rew_cycle               = [];
rew_iti                 = [];

for iCyc = 1:length(cyc.cOn)
    cycle_idx = reward_time > cyc.cOn(iCyc) & reward_time < cyc.cEnd(iCyc);
    
    if iCyc < length(cyc.cOn)
        iti_idx         = reward_time > cyc.cEnd(iCyc) & reward_time < cyc.cOn(iCyc+1);
    else
        iti_idx         = reward_time > cyc.cEnd(iCyc);
    end
        
    rew_cycle           = [rew_cycle individual_reward(cycle_idx)];
    rew_iti             = [rew_iti individual_reward(iti_idx)];
end

ax8                  	= axes('Position', [colmn(1) height(2) dim]); hold on
bp                      = bar([sum(rew_cycle) sum(rew_iti)]);
bp.FaceColor            = [.1 .1 .1];
ax8.XTick             	= [1 2];
ax8.XTickLabel        	= {'Target','Cycle'};
ax8.XLabel.String     	= 'Reward category';
ax8.YLabel.String    	= 'Reward [ml]';
ax8.FontSize          	= fsz;
...
% T =  MW_getEyeHeatmap(data);

% FIX BREAKS (absolute [#] and relative [%]) per coherence
ax10                  	= axes('Position', [colmn(4) height(3) dim]); hold on
% fixbreak_sum        	= sum(strcmp(outcome, 'FixationBreak'));
% Check coherence level at time of fixbreak
% bar plot with fraction of breaks
% bar plot with absolute breaks
...
for iTarget = 1:length(outcome)
    iState                       	= find(outcome_ts(iTarget) >= state_on, 1,'last');              % Find target state
    ...
    t.trg_ts{iState}                = [t.trg_ts{iState} outcome_ts(iTarget)];                       % Extract target timestamp
...
%     t.trg_hit{iState}               = [t.trg_hit{iState} , strcmp(tmp_outcome, 'hit')];           % Indicate if hit or miss
    ...
    smple                           = find(d.time(idx.JS_dir) <= outcome_ts(iTarget), 1, 'last');  	% Extract last JS sample before target
...
%     if strcmp(tmp_outcome, 'hit')
    if strcmp(outcome(iTarget), 'hit')
        rew_idx                     = find(rew_ts > outcome_ts(iTarget),1,'first');                 % Get reward score
...
addpath /Users/anazari/Desktop/CPR
