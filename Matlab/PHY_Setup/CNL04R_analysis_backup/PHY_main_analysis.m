...
% Converts a 'state' struct into a table with spike times at the end.
% Applies inclusion masks: excluded trials become empty cells {}.

p = inputParser;
addParameter(p,'RemoveEmptyUnits',true,@islogical);
parse(p,varargin{:});
removeEmpty = p.Results.RemoveEmptyUnits;

Ntrials = numel(state.cpr_solo);  % assume all trial-wise fields match this length

% -------------------- 1. Identify fields --------------------
allFields = fieldnames(state);

% fields to ignore initially (nested structs)
ignoreFields = {'spk_ts','spk_n','include'};

% trial-wise fields
dataFields = setdiff(allFields, ignoreFields);
...
for i = 1:numel(dataFields)
    ...
    % only accept trial-wise vectors or cell arrays
    if (isvector(val) && numel(val)==Ntrials) || (iscell(val) && numel(val)==Ntrials)
        T.(f) = val(:);  % convert to column
    else
        warning('Skipping field %s (unsupported size/type)', f);
    end
end
...
if isfield(state,'spk_ts')
    unitNames = fieldnames(state.spk_ts);
...
% -------------------- 5. Remove units with no valid trials --------------------
if removeEmpty && isfield(state,'include')
    keepUnit = false(size(unitNames));
    for i = 1:numel(unitNames)
        u = unitNames{i};
        keepUnit(i) = any(state.include.(u));
    end
    T(:, unitNames(~keepUnit)) = [];
    unitNames = unitNames(keepUnit);
end

% -------------------- 6. Move spike units to the end --------------------
