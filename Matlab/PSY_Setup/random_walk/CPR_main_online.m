...
param.walk_duration_ms      	= cell2mat(d.value(d.event == 'CTRL_cycle_duration_ms'));    	% Duration of random walk (stimulus cycle)
param.coh_duration_ms       	= cell2mat(d.value(d.event == 'CTRL_coh_block_duration_ms'));  	% Duration of coherence block
param.polar_step_size       	= cell2mat(d.value(d.event == 'CTRL_polar_step_size'));         % Step size of random walk in polar space
param.reward_probability       	= cell2mat(d.value(d.event == 'CTRL_feedback_probability')); 	% Probability of reward target appearance
param.feedback_interval_ms      = cell2mat(d.value(d.event == 'CTRL_feedback_ITI_ms'));      	% Interval between feedback presentation
...
STIM                            = CPR_create_random_walk();                 % Draw RDP stimulus parameters
